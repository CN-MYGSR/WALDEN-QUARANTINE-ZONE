...
this.crew={ name:this.name, team, kills:0, deaths:0, score:0, isPlayer:false, isCrew:true, pos:this.pos, alive:true, lastFiredT:-99, damage(){}, vel:V3() };
 this.cannonCd=0; this.mgT=0; this.mgBurst=0; this.mgPause=0; this.atgmCd=0;
...
else if(role==='gunner'){ cm.inTurret=true; cm.local.set(0.4,0.32,-0.2); cm.exposed=!!D.openTop; }
else if(role==='loader'){ cm.inTurret=true; cm.local.set(-0.45,0.32,-0.35); cm.exposed=!!D.openTop; }
else { cm.inTurret=true; cm.local.set(-0.55,0.72,-0.5); cm.exposed=true; } // 车长探头
cm.hp=65;
// AI 可感知的代理实体
...
this.grp.add(wh); this.wheels.push(wh);
  ...
  this.turretG=new THREE.Group(); this.turretG.position.y=D.bmpt?1.62:(D.rus?1.86:1.78); this.grp.add(this.turretG);
  if(D.bmpt){
  // BMPT-72: 低矮双联30mm炮塔 + 两侧导弹发射器 + 首上爆反
  const tb=new THREE.Mesh(new THREE.BoxGeometry(2.4,0.38,2.3),this.matBody); tb.position.y=0.04; this.turretG.add(tb);
  const tfront=new THREE.Mesh(new THREE.BoxGeometry(2.4,0.32,0.35),this.matBody); tfront.position.set(0,0.04,1.16); tfront.rotation.x=0.3; this.turretG.add(tfront);
  for(const sx of [-1.05,1.05]){
  for(let i=0;i<2;i++){
  const pod=new THREE.Mesh(new THREE.CylinderGeometry(0.085,0.085,0.95,8),matB);
  pod.rotation.x=HPI; pod.position.set(sx,0.42,-0.15+i*0.62); this.turretG.add(pod);
  }
  }
  const rws=new THREE.Mesh(new THREE.BoxGeometry(0.5,0.22,0.42),this.matBody); rws.position.set(0,0.5,0.18); this.turretG.add(rws);
  const cup=new THREE.Mesh(new THREE.CylinderGeometry(0.28,0.3,0.26,10),this.matBody); cup.position.set(0.6,0.48,-0.55); this.turretG.add(cup);
  ...
  // 车体首上爆反装甲块
  const era=new THREE.MeshLambertMaterial({color:0x2e3328});
  for(const sx of [-0.9,-0.3,0.3,0.9]){
  const eh=new THREE.Mesh(new THREE.BoxGeometry(0.42,0.07,0.2),era);
  eh.position.set(sx,1.42,L/2-0.35); eh.rotation.x=-0.62; this.grp.add(eh);
  }
  } else if(D.rus){
 // 俄式圆顶炮塔 (T-90 风格): 半球铸钢炮塔 + 爆反装甲
 const dome=new THREE.Mesh(new THREE.SphereGeometry(1.18,12,10,0,TAU,0,1.2),this.matBody);
 dome.scale.set(1,0.7,1.28); dome.position.y=0.42; this.turretG.add(dome);
 const turB=new THREE.Mesh(new THREE.BoxGeometry(2.3,0.5,2.7),this.matBody); turB.position.y=0.06; this.turretG.add(turB);
 const hat=new THREE.Mesh(new THREE.CylinderGeometry(0.3,0.3,0.06,10),matB); hat.position.set(0.5,0.86,-0.4); this.turretG.add(hat);
 const cup=new THREE.Mesh(new THREE.CylinderGeometry(0.34,0.38,0.3,10),this.matBody); cup.position.set(-0.55,0.9,-0.5); this.turretG.add(cup);
 const ant=new THREE.Mesh(new THREE.CylinderGeometry(0.008,0.014,1.6,4),matB);
 ant.position.set(-0.9,1.2,-0.6); ant.rotation.z=0.12; this.turretG.add(ant);
 // 炮塔正面爆反装甲块
 const era=new THREE.MeshLambertMaterial({color:0x2e3328});
 for(const sx of [-1.0,-0.5,0,0.5,1.0]){
 const eb=new THREE.Mesh(new THREE.BoxGeometry(0.34,0.07,0.18),era);
 eb.position.set(sx,0.62,1.28); this.turretG.add(eb);
 }
 // 车体首上爆反装甲块
 for(const sx of [-0.9,-0.3,0.3,0.9]){
 const eh=new THREE.Mesh(new THREE.BoxGeometry(0.42,0.07,0.2),era);
 ...
 const cup=new THREE.Mesh(new THREE.CylinderGeometry(0.34,0.38,0.28,10),this.matBody); cup.position.set(-0.55,0.56,-0.5); this.turretG.add(cup);
 // 天线
 const ant=new THREE.Mesh(new THREE.CylinderGeometry(0.008,0.014,1.6,4),matB);
 ant.position.set(-0.9,0.9,-0.6); ant.rotation.z=0.12; this.turretG.add(ant);
 }
 this.pitchG=new THREE.Group(); this.pitchG.position.set(0,0.15,1.5); this.turretG.add(this.pitchG);
 const mant=new THREE.Mesh(new THREE.BoxGeometry(0.85,0.6,0.4),this.matBody); this.pitchG.add(mant);
 if(D.bmpt){
 // 双联 30mm 机关炮 (上下并列)
 for(const by of [0.21,0.35]){
 const tbar=new THREE.Mesh(new THREE.CylinderGeometry(0.048,0.06,D.barrelL,8),matB);
 tbar.position.set(0,by-0.24,D.barrelL/2); tbar.rotation.x=HPI; this.pitchG.add(tbar);
 }
 } else {
...
return;
 ...
 this.cannonCd-=dt; this.mgT-=dt; this.mgPause-=dt; this.atgmCd=(this.atgmCd||0)-dt;
this.crewUpdate(dt);
// 驾驶员阵亡: 车辆瘫痪直至替补入座
...
if(this.playerDriven) (typeof shakeVehicle==='function' ? shakeVehicle(0.28) : addTrauma(0.28));
continue;
}
if(d2>1e-8){
...
let sc=90-d*0.9;
if(d<30) sc+=110;
const atThreat=s.cls===3||(s.atn||0)>0||(s.atNades||0)>0;
...
if(best){
if(best!==this.tTarget){ this.aimReact=rand(0.7,1.3)*EFF_DIFF.react*2+0.35; this.losOK=false; }
...
if(this.losOK&&SMOKES.length&&smokeBlocksLOS(o.x,o.y,o.z,tgt.pos.x,tgt.pos.y+1.2,tgt.pos.z)) this.losOK=false;
 ...
 // BMPT: 对坦克目标补发反坦克导弹
 if(this.def.bmpt&&isTank&&dh<200&&this.atgmCd<=0&&Math.abs(angDiff(this.turretYaw,angDiff(this.yaw,aimYaw)))<0.08){
 this.fireCannon(null,true);
 }
...
if(--this.mgBurst<=0) this.mgPause=rand(0.6,1.2);
 ...
 fireCannon(customDir,rocket){
 if(rocket){
 if(this.atgmCd>0) return;
 this.atgmCd=6;
 } else {
 ...
 if(this.playerDriven) (typeof shakeVehicle==='function' ? shakeVehicle(rocket?0.3:0.5) : addTrauma(rocket?0.3:0.5));
 this.suspPV-=0.55*Math.cos(this.turretYaw);
 this.suspRV+=0.4*Math.sin(this.turretYaw);
  if(rocket){
  // BMPT: 9M120 反坦克导弹 (低重力弹道, 高穿深)
  shells.push({pos:origin.clone(),vel:dir.clone().multiplyScalar(105),team:this.team,owner:shooter,life:4.5,kind:'at',dmgV:940,pen:285,trail:0,grav:1.2,veh:true,rec:[{x:origin.x,y:origin.y,z:origin.z,t:nowT}]});
  } else {
  shells.push({pos:origin.clone(),vel:dir.clone().multiplyScalar(130),team:this.team,owner:shooter,life:3.5,kind:this.def.heavy?'heavy':'tank',dmgV:this.def.dmg,pen:this.def.pen||110,trail:0,veh:true,rec:[{x:origin.x,y:origin.y,z:origin.z,t:nowT}]});
  }
...
addTrauma(0.02); // 同轴机枪常规抖动, 不走 shakeVehicle(频率太高, 不打扰)
}
takeDmg(amt,attacker){
if(!this.alive) return;
this.hp-=amt;
this.lastDmgT=nowT;
if(this.playerDriven){ dmgFlash=Math.min(1,dmgFlash+0.35); (typeof shakeVehicle==='function' ? shakeVehicle(0.3) : addTrauma(0.3)); }
...
if(s.life<=0){ shells.splice(i,1); continue; }
 ...
 const dir=s.vel.clone().normalize();
  s.vel.y-=(s.grav||3.5)*dt;
 s.trail-=dt;
 if(s.rec&&s.rec.length<240) s.rec.push({x:s.pos.x,y:s.pos.y,z:s.pos.z,t:nowT});
...
const p=V3(s.pos.x+dir.x*hitD,s.pos.y+dir.y*hitD,s.pos.z+dir.z*hitD);
 if(hitApc){
 if(s.veh&&s.rec&&s.owner&&s.owner.isPlayer) xrayTrigger(s,p,true,hitApc.name||'APC');
...
// ===== 击穿模拟 =====
 ...
 if(s.veh&&s.rec&&s.owner&&s.owner.isPlayer) xrayTrigger(s,p,res.pen,hitTank.name);
...
for(const s of combatants){
if(!s.alive||s.onVehicle) continue;
// 不误伤队友/同单位, 但炸自己会受伤 (单位判定见 47_units)
if(s!==attacker&&(s.team===team||(attacker&&typeof unitBlocks==='function'&&unitBlocks(attacker,s)))) continue;
...
if(player.alive&&p.distanceTo(camera.position)<18) (typeof shakeExplosion==='function' ? shakeExplosion(0.5, p.distanceTo(camera.position)) : addTrauma(0.5));
}
