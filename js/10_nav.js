...
// 最佳努力: 记录搜索过程中"离目标最近"的已展开节点。当目标不可达(或搜索被 guard 截断)时,
// 退化为朝该节点的部分路径, 让 Bot 至少朝目标方向推进, 而不是原地卡死。
// 这正是"NPC 一直在出生点不动"的主因: 长距离(>50m)寻路失败率约 16%, 失败即 path=null。
let bestNode=startI, bestH=Math.hypot(si-ti,sj-tj);
while(hLen>0&&guard++<60000){
const cur=pop();
if(cur===goalI){ found=true; break; }
const ci2=cur%N, cj2=(cur/N)|0;
const ch=Math.hypot(ci2-ti,cj2-tj);
if(ch<bestH){ bestH=ch; bestNode=cur; }
const cI=ci2, cJ=cj2, cg=gScore[cur];
for(let d=0;d<8;d++){
...
let goal=goalI;
if(!found){
// 完全没推进(起点即孤立): 直连兜底, 由 Bot 的 moveDir 贴墙滑行处理
if(bestNode===startI) return [[tx,tz]];
goal=bestNode; // 部分路径: 走到搜索能达到的、离目标最近的点
}
const path=[];
let cur=goal;
while(cur!==startI&&cur>=0){ const i=cur%N,j=(cur/N)|0; const [x,z]=toW(i,j); path.push([x,z]); cur=came[cur]; }
path.reverse();
// 只有真正抵达目标时才补上精确终点; 部分路径保留"可达前沿"作为终点
if(found) path.push([tx,tz]);
// 拉绳平滑: 视线可达则跳过中间路点
const sm=[];
let ax=sx, az=sz, k=0;
while(k<path.length){
