...
ads:{ pos:V3(0,-0.077,-0.3), rot:V3(0,0,0) },
...
// 机瞄模型离镜头太近时，照门和枪机的低模几何会放大成一整块黑墙，
// 尤其是手枪和冲锋枪。ADS 只把非光学瞄具的视模型向前推远，
// 不改变真实相机、弹道或准星投射；这样准星仍在屏幕中心，但枪体不会堵住目标。
//
// 病因说明(2026-09 复修): 光靠"往前推固定距离"治不干净 —— 各枪长度差很多,
// 长枪的枪托/枪机尾部在 ADS 时能落到离镜头 0.1m 出头, 透视放大后依然糊成中央黑块。
// 现在改成双保险: ① 基础推远量加大; ② 换枪时实测枪身+双臂的包围盒, 反算出 ADS 目标深度的下界
// (VM.adsZLimit), 取"更远的那个"作为 ADS 稳态深度 —— 保证枪上任何一点离镜头都 >= VM_ADS_MIN_CLEAR。
// 注意是约束"目标深度"而不是事后钳制 root.z: 后者会在开镜过渡中产生可见跳变。
const VM_ADS_MIN_CLEAR=0.28;
function adsViewDepth(d,key){
if(d.scoped) return 0;       // 光学镜 ADS 后由 scopeOv 接管，保持原有镜内遮罩逻辑
// 基础推远量 (历史实测: 0.14 时枪体暗块已明显收缩, 但 0.20 只对短枪够用, 长枪枪托仍会顶到脸前)
let push = d.rocket ? 0.16 : (d.atRifle ? 0.22 : 0.30);
// 改装瞄具(反射/2倍/夜视)在 addModVisuals 里把 ADS 锚点朝镜头拉近了 0.05~0.06,
// 这里补回同样的距离, 否则一装镜, 镜筒又会贴到镜头上重新糊成黑块。
try{
  if(key && typeof effectiveOptic==='function'){
    const op=effectiveOptic(key);
    if(op && op!=='optic_iron') push += 0.06;
  }
}catch(e){}
return push;
}
// 换枪时实测: 在"ADS 姿态"下(横向/纵向偏移照实施加, 只把 z 归零), 找出会落进
// 屏幕中央视野走廊的那些部件, 取其中离镜头最近的那个深度 zMaxCenter。
// 推导: 该部件离镜头最近时世界 z = VM.root.position.z + zMaxCenter, 要求 <= -VM_ADS_MIN_CLEAR,
//       即 root.z <= -(VM_ADS_MIN_CLEAR + zMaxCenter)。返回的就是这个下界 (VM.adsZLimit)。
// 为什么不量整把枪: 枪托/背带在 ADS 时本来就贴着腮帮子(离眼十几厘米), 把它算进去会把
//       整把枪推到 0.7m 开外, 准星反而小得看不清。真正病因只有"中央那一条"被放大。
// 注意: 必须自己遍历并跳过 visible===false 的部件 —— Box3.setFromObject 会把隐藏几何
//       也算进去, 火箭筒的待装填弹(初始 z≈0.78)会把推远量顶到夸张的数值。
const VM_ADS_CENTER_X=0.06, VM_ADS_CENTER_Y=0.06;
function _vmAccumCenterZ(obj,acc){
...
if(bb){
const b=bb.clone().applyMatrix4(obj.matrixWorld);
const hitX=(b.min.x<=VM_ADS_CENTER_X&&b.max.x>=-VM_ADS_CENTER_X);
const hitY=(b.min.y<=VM_ADS_CENTER_Y&&b.max.y>=-VM_ADS_CENTER_Y);
if(hitX&&hitY&&b.max.z>acc.z){ acc.z=b.max.z; acc.hit=true; }
}
}catch(e){}
}
if(obj.children) for(let i=0;i<obj.children.length;i++) _vmAccumCenterZ(obj.children[i],acc);
}
function measureAdsZLimit(){
const g=VM.gunParts&&VM.gunParts.gun;
if(!g||typeof THREE==='undefined'||!THREE.Box3) return null;
const rp=VM.root.position.clone(), rr=VM.root.rotation.clone();
const sp=VM.sway.position.clone(), sr=VM.sway.rotation.clone();
VM.root.position.set((VM.ads&&VM.ads.pos)?VM.ads.pos.x:0,(VM.ads&&VM.ads.pos)?VM.ads.pos.y:0,0);
VM.root.rotation.set(0,0,0);
VM.sway.position.set(0,0,0); VM.sway.rotation.set(0,0,0);
VM.root.updateMatrixWorld(true);
const acc={z:-Infinity,hit:false};
try{
_vmAccumCenterZ(g,acc);
if(VM.arms){ _vmAccumCenterZ(VM.arms.L,acc); _vmAccumCenterZ(VM.arms.R,acc); }
}catch(e){}
VM.root.position.copy(rp); VM.root.rotation.copy(rr);
VM.sway.position.copy(sp); VM.sway.rotation.copy(sr);
VM.root.updateMatrixWorld(true);
if(!acc.hit||!isFinite(acc.z)) return null;
return -(VM_ADS_MIN_CLEAR+acc.z);
}
...
// ===== 机瞄自动校准 =====
// 问题: 各枪的 ads 锚点高度是手填的, 与枪模型里真正的准星顶(前准星/照门)常常差
// 0.005~0.015 世界单位。因为相机射线恒指屏幕正中, 这点差值会让开镜后"瞄具偏高、
// 准星看起来偏低"(实测 m1911/g17/mp40/stg44/ak74m/m4/kar98 普遍偏高 10~12px)。
// 方案: 模型建好后, 用一个不可见的测量探针锚定到准星顶, 读取其局部 Y,
//       据此把 ads.pos.y 修正为 -(准星Y), 使开镜时准星精确落在屏幕中心。
// 带瞄镜(scoped)的武器不参与(其 ads 锚点对准的是镜筒, 由瞄准镜覆盖层负责)。
if(VM.ads&&VM.ads.pos&&!(WPN_DEFS[key]||{}).scoped&&!(WPN_DEFS[key]||{}).mortar){
const sa=VM.gunParts&&VM.gunParts.sightAnchor;
if(sa&&isFinite(sa.y)){
// sa.y = 准星顶在枪局部坐标下的高度; ads.pos.y 应等于其相反数
const want=-sa.y;
const dy=want-VM.ads.pos.y;
if(Math.abs(dy)>0.0005){
VM.ads={pos:V3(VM.ads.pos.x,VM.ads.pos.y+dy,VM.ads.pos.z),rot:VM.ads.rot};
}
VM.ads._calibrated=true;
}
}
VM.state='draw'; VM.stateT=0; VM.stateDur=0.45;
positionHands(key);
// 换枪时算一次 ADS 深度上限 (见 measureAdsZLimit 的推导), 之后每帧只做一次比较
VM.adsZLimit=null;
try{ VM.adsZLimit=measureAdsZLimit(); }catch(e){}
...
// 开镜速度: 默认 1/14 秒时间常数; 若武器定义了 adsTime(如 CQB 平衡后的狙击枪), 按其换算。
// k = 4.6 / adsTime  →  adsTime 秒时完成约 99% 的开镜过渡。
const adsK = d.adsTime>0 ? clamp(4.6/d.adsTime, 2, 26) : 14;
VM.adsBlend=dampF(VM.adsBlend,adsTgt,adsK,dt);
const sprTgt=(player.sprinting&&VM.state==='idle')?1:0;
VM.sprintBlend=dampF(VM.sprintBlend,sprTgt,8,dt);
// ===== 高低姿态持枪混合 =====
// 低位持枪: 枪身下沉 + 枪口下压(rotation.x 增大) + 向左下收拢, 呈"警戒行军"姿态。
// ADS / 架枪 / 趴下 时强制回高位(否则瞄具无法对齐眼睛)。
const carryLowTgt=((player.carry==='low')&&!player.ads&&!player.braced&&!player.prone&&VM.state!=='melee')?1:0;
VM.carryBlend=dampF(VM.carryBlend||0,carryLowTgt,7,dt);
const lb=VM.carryBlend*(1-VM.adsBlend);
...
let adsPosZ=VM.ads.pos.z-adsViewDepth(d,VM.key);
// ADS 稳态最小净空: 取"基础推远量"与"实测净空要求"里更远的一个。
// 直接约束目标深度(而不是事后钳制 root.z)才不会在过渡中产生跳变。
// 光学镜(d.scoped)不参与: 镜筒本来就该贴到眼睛前, 由 scopeOv 接管, adsViewDepth 也返回 0。
if(!d.scoped&&VM.adsZLimit!=null&&adsPosZ>VM.adsZLimit) adsPosZ=VM.adsZLimit;
const pz=lerp(hip.pos.z,adsPosZ,VM.adsBlend);
...
// ===== 低位持枪姿态偏移 =====
// 下压枪口(rx+), 枪身下沉(oy-), 略微后收(oz+)并内收(ox-), 形成"低姿戒备"
if(lb>0.001){
  rx+=lb*(d.rocket?0.30:0.34);      // 枪口明显下压
  ry+=lb*(d.pistol?-0.10:-0.14);    // 枪身内旋(指向斜下前方)
  rz-=lb*0.10;
  oy-=lb*(d.rocket?0.045:0.055);
  oz+=lb*0.03;
  ox-=lb*(d.pistol?0.03:0.045);
}
...
VM.nadeM.position.set(0.14-bk*0.22,-0.12+k*0.04-bk*0.10,-0.3+k*0.05+bk*0.5);
VM.nadeM.rotation.set(c*0.5,0,0);
handROverride={pos:V3(0.13-bk*0.18,-0.15-bk*0.07,-0.22+bk*0.45),rot:V3(0.5+c*0.8,0,0)};
...
if(k>0.88) vmPlayOnce('b4',()=>AudioSys.click(1300,0.22,0.03));
    ...
    // 摄像机坐标上抬(视模型距镜头约0.4米, 1080p下约0.015世界单位)
    // 修正: 该抬高必须随开镜淡出。原先无条件施加, 导致开镜时整枪被抬高约 10~30px,
    // 瞄具落到屏幕中心之上, 于是"准星看起来偏低"(实测偏差 10~11px @1366x768)。
    // 现在乘 (1-adsBlend): 腰射时保留抬枪手感, 开镜时机瞄严格对齐准星。
    const VM_RAISE_Y=0.015*(1-VM.adsBlend);
...
// 基础 FOV 来自设置 (默认 74); 开镜 FOV 按同比例缩放 → 设置只改"视野", 不改开镜倍率。
const _baseFov=(SETTINGS&&SETTINGS.fov)||74;
const _adsFovTgt=(d.adsFov||60)*(_baseFov/BASE_FOV_74);
const tgtFov=lerp(_baseFov, _adsFovTgt, VM.adsBlend)+(player.sprinting?4*VM.sprintBlend:0);
camera.fov=dampF(camera.fov,tgtFov,16,dt);
camera.updateProjectionMatrix();
}
function vmFireKick(){
const d=WPN_DEFS[VM.key];
VM.recoilV+=d.recoil*(3.2+rand(-0.4,0.4));
VM.kickV+=d.kick*22;
