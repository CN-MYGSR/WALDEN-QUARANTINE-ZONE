...
this.grp=new THREE.Group();
 ...
 if(D.rus){
 // 乌拉尔-4320: 平头驾驶室 + 帆布车棚
 const cab2=new THREE.Mesh(new THREE.BoxGeometry(2.0,1.5,1.3),this.matBody); cab2.position.set(0,1.65,1.35); this.grp.add(cab2);
 const grill=new THREE.Mesh(new THREE.BoxGeometry(1.7,0.55,0.15),new THREE.MeshLambertMaterial({color:0x2c3034}));
 grill.position.set(0,1.05,2.05); this.grp.add(grill);
 const winShield2=new THREE.Mesh(new THREE.BoxGeometry(1.8,0.55,0.06),new THREE.MeshLambertMaterial({color:0x2c3438}));
 winShield2.position.set(0,1.95,2.03); winShield2.rotation.x=-0.2; this.grp.add(winShield2);
 const tarp=new THREE.MeshLambertMaterial({color:0x5a5238});
 for(let i=0;i<4;i++){
 const hoop=new THREE.Mesh(new THREE.CylinderGeometry(0.04,0.04,2.15,8),tarp);
 hoop.rotation.x=HPI; hoop.position.set(0,2.05,-0.25-i*0.85); this.grp.add(hoop);
 }
 } else {
