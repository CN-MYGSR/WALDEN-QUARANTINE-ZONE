...
const entry={parts,cons,t:GAMEMODE==='extract'?50:9,head:headRes.pe,helmCon,stolenFrom:bot};
RAG.list.push(entry);
return entry;
}
...
const limbOf=(relH)=>{
if(relH>0.95) return 'arms';
if(relH>0.45) return 'torso';
return 'legs';
};
...
// 同队 或 同单位 都不吃这颗子弹 (单位见 47_units; 未加载时退化为只按队伍)
if(shooter&&(s.team===shooter.team||(typeof unitBlocks==='function'&&unitBlocks(shooter,s)))) continue;
...
if(px*px+py*py+pz*pz<0.26*0.26){ best={sol:s,t:tProj,head:true,part:'head'}; bestT=tProj; continue; }
}
...
if(r){ const relH=r.t?o.y+dir.y*r.t-s.pos.y:s.pos.y; best={sol:s,t:r.t,head:false,part:limbOf(relH)}; bestT=r.t; }
}
return best;
}
const rockets=[];
