...
// 近轴对齐阈值 0.09 (≈5.2°): 城市街区/道具的微旋转(±3.5°)全部走单块 AABB 碰撞体。
// 旧阈值 0.02 会把每一面微旋转的墙拆成 ~7 个 0.55m 方块 —— 等效墙厚被撑到 0.55m,
// 1.3m 门洞被吃掉近半(寻路与玩家都过不去), 同时碰撞体数量放大 ~7 倍。
if(Math.abs(sn)<0.09||Math.abs(cs)<0.09){
...
if(player.alive&&Math.hypot(g.cx-player.pos.x,g.cz-player.pos.z)<25) (typeof shakeExplosion==='function' ? shakeExplosion(0.4, Math.hypot(g.cx-player.pos.x,g.cz-player.pos.z)) : addTrauma(0.4));
}
// 承重墙被毁 → 屋顶随之坍塌
...
// ===================== CQB 破旧街道设施库 (阴暗街区近战) =====================
// 设计要点: 全部为"低矮/半身掩体"或"视线遮挡物", 高度介于 0.8~1.6m,
// 保证站位可射击、蹲下完全隐蔽; 碰撞体与掩体点(CYLS/coverPoints)同步注册。
// 材质走 THEME.cqb 低饱和调色(见 01_data.js), 缺省时回退到旧材质, 保证兼容。

// 生锈集装箱: 2.4×2.6×6.0, 顶面可站(竖直作战), 侧面全遮挡
function shippingContainer(x,z,ry,collapsed){
const g0=heightAt(x,z);
collapsed=collapsed||false;
const w=2.44, h=collapsed?1.35:2.6, d=6.06;
const body=solidBox(MAT.containerRust||MAT.metalDark,w,h,d,x,g0+h/2,z,ry);
if(!collapsed){
// 顶部加强框(可站立)
mesh(new THREE.BoxGeometry(w+0.1,0.12,d+0.1),MAT.containerTop||MAT.metal, x,g0+h+0.06,z,ry);
// 端部双开舱门(一门半开, 形成可绕行缺口)
const cs=Math.cos(ry),sn=Math.sin(ry);
const doorY=g0+h/2;
mesh(new THREE.BoxGeometry(0.1,h*0.94,w*0.5),MAT.containerDoor||MAT.metalDark, x+sn*(d/2),doorY,z+cs*(d/2), ry+0.62);
// 侧面加强肋
for(let i=-1;i<=1;i++){
mesh(new THREE.BoxGeometry(0.07,h*0.9,0.16),MAT.containerRust||MAT.metalDark, x+cs*i*1.9+sn*(w/2),g0+h/2,z-sn*i*1.9+cs*(w/2), ry);
}
} else {
// 坍塌箱: 侧壁外翻成斜板, 提供斜坡与低掩体
const cs=Math.cos(ry),sn=Math.sin(ry);
const flap=mesh(new THREE.BoxGeometry(w*0.9,h*0.5,d),MAT.containerRust||MAT.metalDark, x+cs*(w*0.5),g0+h*0.35,z-sn*(w*0.5), ry);
flap.rotation.z=0.78;
addCoverAround(x,z,w,d,ry);
}
coverPoints.push({x:x+Math.cos(ry)*(w/2+0.9),z:z-Math.sin(ry)*(w/2+0.9)});
coverPoints.push({x:x-Math.cos(ry)*(w/2+0.9),z:z+Math.sin(ry)*(w/2+0.9)});
coverPoints.push({x:x+Math.sin(ry)*(d/2+0.9),z:z+Math.cos(ry)*(d/2+0.9)});
}

// 废弃轿车: 低矮掩体, 车身趴伏, 引擎盖/车顶两级掩体
function wreckCar(x,z,ry){
const g0=heightAt(x,z);
solidBox(MAT.carRust||MAT.metalDark,1.85,0.78,4.35,x,g0+0.62,z,ry);
// 车顶(略矮, 半身高)
const cab=solidBox(MAT.carRust||MAT.metalDark,1.7,0.62,2.1,x-Math.sin(ry)*0.35,g0+1.3,z-Math.cos(ry)*0.35,ry);
cab.rotation.z=rand(-0.05,0.05);
// 破碎挡风玻璃
const ws=mesh(new THREE.BoxGeometry(1.6,0.5,0.06),MAT.glassCrack||MAT.glass,x+Math.sin(ry)*0.95,g0+1.42,z+Math.cos(ry)*0.95, ry);
ws.rotation.x=rand(-0.4,0.4);
// 车轮(部分缺失)
const cs=Math.cos(ry),sn=Math.sin(ry);
for(const o of [-1.35,1.35]) for(const s of [-1,1]){
if(Math.random()<0.28) continue;
const wh=mesh(new THREE.CylinderGeometry(0.34,0.34,0.22,10),MAT.metalDark, x+cs*s*0.85+sn*o,g0+0.34,z-sn*s*0.85+cs*o, ry);
wh.rotation.z=HPI;
}
// 敞开的引擎盖
const hood=mesh(new THREE.BoxGeometry(1.5,0.08,1.0),MAT.carRust||MAT.metalDark, x+Math.sin(ry)*1.75,g0+1.22,z+Math.cos(ry)*1.75, ry);
hood.rotation.x=-0.5;
addCoverAround(x,z,2.0,4.5,ry);
}

// 破损路障: 混凝土隔离墩 / 反坦克拒马 / 铁马护栏, 三种形态
function barrierConcrete(x,z,ry){
const g0=heightAt(x,z);
// 交错的混凝土墩排
for(let i=-1;i<=1;i++){
const cs=Math.cos(ry),sn=Math.sin(ry);
const o=i*2.0;
const m=solidBox(MAT.concreteBarrier||MAT.stone,1.5,0.95,0.62, x+sn*o,g0+0.48,z+cs*o, ry+rand(-0.1,0.1));
m.rotation.z=rand(-0.03,0.03);
}
coverPoints.push({x,z},{x:x+Math.sin(ry)*1.4,z:z+Math.cos(ry)*1.4});
}
function barrierSteel(x,z,ry){
const g0=heightAt(x,z);
// 铁马护栏: 管架结构, 可蹲射
for(let i=-1;i<=1;i++){
const sn=Math.sin(ry),cs=Math.cos(ry),o=i*1.6;
mesh(new THREE.BoxGeometry(0.07,1.05,0.07),MAT.metalDark, x+sn*o+cs*0.7,g0+0.52,z+cs*o-sn*0.7, ry);
mesh(new THREE.BoxGeometry(0.07,1.05,0.07),MAT.metalDark, x+sn*o-cs*0.7,g0+0.52,z+cs*o+sn*0.7, ry);
mesh(new THREE.BoxGeometry(1.6,0.08,0.08),MAT.metal, x+sn*o,g0+1.0,z+cs*o, ry+HPI);
mesh(new THREE.BoxGeometry(1.6,0.08,0.08),MAT.metal, x+sn*o,g0+0.35,z+cs*o, ry+HPI);
}
addBoxCollider(x,g0+0.52,z, Math.abs(Math.sin(ry))>0.5?1.4:4.8, 1.05, Math.abs(Math.sin(ry))>0.5?4.8:1.4);
coverPoints.push({x,z});
}
// 堆叠杂物: 托盘/纸箱/油桶/塑料筐混堆, 制造巷内碎片化掩体
function clutterPile(x,z,ry,scale){
const g0=heightAt(x,z);
scale=scale||1;
const n=randi(3,6);
for(let i=0;i<n;i++){
const a=rand(0,TAU), rr=rand(0.3,1.5)*scale;
const px=x+Math.sin(a)*rr, pz=z+Math.cos(a)*rr;
const s=rand(0.4,0.85)*scale;
const pick=Math.random();
const gy=heightAt(px,pz);
if(pick<0.4) solidBox(MAT.cardboard||MAT.woodDark,s,s*0.8,s, px,gy+s*0.4,pz, rand(0,TAU));
else if(pick<0.7) solidBox(MAT.woodPallet||MAT.wood,s*1.3,s*0.22,s*1.1, px,gy+s*0.22,pz, rand(0,TAU));
else mesh(new THREE.CylinderGeometry(s*0.5,s*0.5,s*0.9,9),MAT.metal, px,gy+s*0.45,pz);
}
addCoverAround(x,z,2.2*scale,2.2*scale,ry);
coverPoints.push({x,z});
}
// 损坏路灯: 倾倒/断臂/裸线, 昼夜差异明显
function brokenLamp(x,z,tilt){
const g0=heightAt(x,z);
tilt=(tilt===undefined)?rand(0,0.3):tilt;
const h=rand(3.6,5.0);
const grp=new THREE.Group(); grp.position.set(x,g0,z); grp.rotation.z=tilt; grp.rotation.y=rand(0,TAU);
const pole=new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.09,h,7),MAT.metalDark);
pole.position.y=h/2; pole.castShadow=true; grp.add(pole);
if(tilt<0.35){
// 灯臂+破裂灯罩
const arm=new THREE.Mesh(new THREE.BoxGeometry(0.07,0.07,0.95),MAT.metalDark);
arm.position.set(0,h-0.15,0.42); arm.castShadow=true; grp.add(arm);
const head=new THREE.Mesh(new THREE.BoxGeometry(0.3,0.12,0.42),MAT.metal);
head.position.set(0,h-0.28,0.8); grp.add(head);
}
world.add(grp);
CYLS.push({x,z,r:0.16,y0:g0,y1:g0+h});
coverPoints.push({x:x+0.7,z});
}
// 积水坑: 低洼反光水面 + 涟漪贴片, 纯视觉(不阻挡)
function puddle(x,z,r){
const g0=heightAt(x,z);
const m=mesh(new THREE.CircleGeometry(r,14), MAT.puddle||MAT.water, x,g0+0.025,z,rand(0,TAU),false,false);
m.rotation.x=-HPI;
m.renderOrder=1;
PUDDLES.push({x,z,r,y:g0,m});
}
// 碎玻璃/瓦砾地面: 平面贴片 + 少量立起碎片, 纯视觉
function glassDebris(x,z,r){
const g0=heightAt(x,z);
const m=mesh(new THREE.CircleGeometry(r,10), MAT.glassGround||MAT.glass, x,g0+0.02,z,rand(0,TAU),false,false);
m.rotation.x=-HPI; m.renderOrder=2;
for(let i=0;i<5;i++){
const a=rand(0,TAU), rr=rand(0,r);
const sh=mesh(new THREE.PlaneGeometry(rand(0.12,0.3),rand(0.1,0.24)),MAT.glass, x+Math.sin(a)*rr,g0+0.09,z+Math.cos(a)*rr,rand(0,TAU),false,false);
sh.rotation.x=-HPI+rand(-0.5,0.5);
}
}
// 沙袋矮墙(巷战版): 更低更碎, 适合蹲射
function coverStack(x,z,ry){
const g0=heightAt(x,z);
const n=randi(2,3);
for(let i=0;i<n;i++){
const sn=Math.sin(ry),cs=Math.cos(ry),o=(i-(n-1)/2)*1.55;
solidBox(MAT.sandbag,1.35,0.72,0.85, x+sn*o,g0+0.36+rand(0,0.12),z+cs*o, ry+rand(-0.08,0.08));
}
coverPoints.push({x:x-Math.sin(ry)*1.3,z:z-Math.cos(ry)*1.3});
coverPoints.push({x:x+Math.sin(ry)*1.3,z:z+Math.cos(ry)*1.3});
}
// 巷道垃圾桶/废料桶堆 (窄巷填充)
function trashBin(x,z,ry){
const g0=heightAt(x,z);
const m=mesh(new THREE.CylinderGeometry(0.36,0.3,0.95,10),MAT.metalDark,x,g0+0.48,z);
m.rotation.z=rand(-0.15,0.15);
mesh(new THREE.CylinderGeometry(0.38,0.38,0.08,10),MAT.metal,x,g0+1.0,z,ry);
CYLS.push({x,z,r:0.4,y0:g0,y1:g0+1.0});
coverPoints.push({x:x+0.8,z});
}
// 天井/巷口拱门: 建筑之间的通行孔洞, 制造视线阻断
function archGate(x,z,ry,w){
w=w||5.2;
const g0=heightAt(x,z);
const cs=Math.cos(ry),sn=Math.sin(ry);
const H=3.6;
solidBox(MAT.brick,0.5,H,0.9, x+cs*(w/2),g0+H/2,z-sn*(w/2), ry);
solidBox(MAT.brick,0.5,H,0.9, x-cs*(w/2),g0+H/2,z+sn*(w/2), ry);
// 拱顶
const top=solidBox(MAT.brick,w+0.9,0.7,1.1, x,g0+H-0.3,z, ry);
addSnowCap(top);
NAV_CLEARS.push({x,z,r:Math.min(2.2,w*0.4)});
coverPoints.push({x:x+cs*(w/2+0.7),z:z-sn*(w/2+0.7)});
coverPoints.push({x:x-cs*(w/2+0.7),z:z+sn*(w/2+0.7)});
}
const PUDDLES=[];
